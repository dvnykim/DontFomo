...
3. **What the project says it is** — `a launchpad, per its own listing`. Attribute it;
   it is the project's own marketing copy, not a finding.
4. **The namesake** — `named after Tesla`. Weak on its own, but when a dozen coins are
...
5. **Nothing.** Write the cap and stop. A bare `$TICKER -> hit $2m` is a complete,
   honest line. Inventing a reason to fill the slot is the failure mode that kills trust.

A coin with nothing under 1–4 is rendered as a single row automatically. You do not need
